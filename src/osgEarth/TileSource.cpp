...
TileSourceFactory::create(const TileSourceOptions& options)
{
    ...
    tileSource = readFile<TileSource>( driverExt );
    if ( !tileSource )
    {
    ...
    {
        OE_DEBUG << LC << "Tile source Profile = " << (tileSource->getProfile() ? tileSource->getProfile()->toString() : "NULL") << std::endl;
